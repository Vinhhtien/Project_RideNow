...
<head>
  ...
  <style>
    /* Layout */
    .broadcast-wrap{display:grid;grid-template-columns: 1.4fr .9fr;gap:1rem}
    @media (max-width: 992px){.broadcast-wrap{grid-template-columns:1fr}}

    /* Form */
    .form-row{display:flex;flex-direction:column;gap:.5rem;margin:.75rem 0}
    .form-row input,.form-row textarea{
      width:100%;padding:.8rem .9rem;border:1px solid var(--border-color);
      border-radius:10px;background:#fff;color:#111;font:inherit
    }
    .form-row textarea{min-height:180px;resize:vertical}
    .row-tools{display:flex;align-items:center;gap:.5rem;justify-content:space-between}
    .hint{color:var(--text-light);font-size:.9rem}

    /* Counters */
    .counter{font-size:.85rem;color:var(--text-light)}
    .counter .ok{color:#16a34a}.counter .warn{color:#f59e0b}.counter .err{color:#dc2626}

    /* Chips */
    .chips{display:flex;flex-wrap:wrap;gap:.5rem;margin-top:.25rem}
    .chip{border:1px solid var(--border-color);background:var(--card-bg);color:var(--text);
      padding:.35rem .6rem;border-radius:999px;cursor:pointer;font-size:.85rem;transition:.15s}
    .chip:hover{transform:translateY(-1px);border-color:var(--primary-color);color:var(--primary-color)}

    /* Actions */
    .actions{display:flex;gap:.6rem;justify-content:flex-end;margin-top:.75rem}
    .btn-ghost{background:transparent;border:1px dashed var(--border-color);color:var(--text)}
    .btn-ghost:hover{border-color:var(--primary-color);color:var(--primary-color)}

    /* Preview card */
    .preview{border:1px solid var(--border-color);border-radius:12px;background:var(--card-bg);padding:1rem}
    .pv-head{display:flex;align-items:center;gap:.6rem;margin-bottom:.35rem}
    .pv-ic{width:36px;height:36px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:#e3f2fd;color:#1976d2}
    .pv-title{font-weight:700}
    .pv-time{font-size:.85rem;color:var(--text-light)}
    .pv-body{white-space:pre-wrap;color:var(--text)}

    /* Recipient (details) */
    details.target{border:1px solid var(--border-color);border-radius:10px;background:var(--card-bg);padding:.75rem 1rem;margin-top:.6rem}
    details.target>summary{list-style:none;cursor:pointer;font-weight:700;display:flex;align-items:center;gap:.5rem}
    details.target>summary::before{content:"\f1d8";font-family:"Font Awesome 6 Free";font-weight:900;color:var(--primary-color)}
    details.target[open]{box-shadow:0 1px 0 rgba(0,0,0,.02) inset}
    .label-sub{font-size:.85rem;color:var(--text-light);margin-top:-.25rem}
  </style>
</head>
<body class="admin">
...
<main class="content">
  ...
  <c:if test="${param.broadcast eq 'ok'}">
    <div class="alert alert-success" role="alert"><i class="fas fa-check-circle"></i> Đã gửi thành công cho ${param.sent} đối tác.</div>
  </c:if>
  <c:if test="${param.broadcast eq 'fail'}">
    <div class="alert alert-error" role="alert"><i class="fas fa-times-circle"></i> Gửi thất bại. Vui lòng thử lại.</div>
  </c:if>
  <c:if test="${param.broadcast eq 'invalid'}">
    <div class="alert alert-warning" role="alert"><i class="fas fa-exclamation-triangle"></i> Vui lòng nhập đủ tiêu đề và nội dung.</div>
  </c:if>
  <c:if test="${param.broadcast eq 'notfound'}">
    <div class="alert alert-warning" role="alert"><i class="fas fa-exclamation-triangle"></i> Không tìm thấy đối tác khớp với tiêu chí người nhận.</div>
  </c:if>
  ...
  <section class="panel">
    ...
    <div class="panel-body">
      <div class="broadcast-wrap">
        <!-- LEFT: Form -->
        <form id="bcForm" method="post" action="${pageContext.request.contextPath}/admin/notify" autocomplete="off" novalidate>
          <!-- Title -->
          <div class="form-row">
            <div class="row-tools">
              <label for="title" style="font-weight:600">Tiêu đề (tối đa 200 ký tự)</label>
            </div>
            <input id="title" name="title" maxlength="200" required placeholder="VD: Bảo trì hệ thống từ 23:00–24:00 đêm nay">
            <div class="chips" aria-label="Mẫu nội dung nhanh">
              <button type="button" class="chip" data-tpl-title="Bảo trì hệ thống" data-tpl-msg="Hệ thống sẽ bảo trì từ 23:00–24:00. Trong thời gian này có thể có gián đoạn tạm thời. Cảm ơn bạn đã phối hợp.">Bảo trì hệ thống</button>
              <button type="button"
                      class="chip"
          ...
          <div class="form-row">
            ...
            <textarea id="message" name="message" required placeholder="Nhập nội dung gửi tới đối tác…"></textarea>
            ...
            <details class="target">
              ...
              <div class="form-row" style="margin-top:.5rem">
                ...
                <div class="label-sub">Nhập <strong>tên người dùng</strong> hoặc <strong>tên công ty</strong>. Để trống để gửi tới tất cả đối tác.</div>
          ...
          <div class="actions">
            <button type="button" class="btn btn-ghost" id="clearDraft"><i class="fas fa-eraser"></i> Xoá nháp</button>
            <a class="btn btn-outline" href="${pageContext.request.contextPath}/admin/dashboard">Huỷ</a>
            <button class="btn btn-primary" type="submit"><i class="fas fa-paper-plane"></i> Gửi thông báo</button>
          </div>
        </form>
...
(function(){
  ...
  try{
    const t = localStorage.getItem(KEY_T); if(t) titleEl.value=t;
    const m = localStorage.getItem(KEY_M); if(m) msgEl.value=m;
  }catch(e){}
  function saveDraft(){
    try{ localStorage.setItem(KEY_T, titleEl.value||''); localStorage.setItem(KEY_M, msgEl.value||''); }catch(e){}
  }

  titleEl.addEventListener('input', ()=>{updateTitleCounter(); updatePreview(); saveDraft();});
  msgEl.addEventListener('input',   ()=>{updateMsgCounter();   updatePreview(); saveDraft();});
  ...
  document.querySelectorAll('.chip[data-tpl-title]').forEach(ch=>{
    ch.addEventListener('click', ()=>{
      ...
      updateTitleCounter(); updateMsgCounter(); updatePreview(); saveDraft();
    });
  });
  ...
  clearBtn?.addEventListener('click', ()=>{
    if(confirm('Xoá nháp hiện tại?')){
      ...
      updateTitleCounter(); updateMsgCounter(); updatePreview();
      try{ localStorage.removeItem(KEY_T); localStorage.removeItem(KEY_M);}catch(e){}
    }
  });
  ...
  setTimeout(()=>document.querySelectorAll('.alert').forEach(a=>{
    a.style.transition='opacity .4s'; a.style.opacity='0'; setTimeout(()=>a.remove(),400);
  }), 5000);
})();
...
(function(){
  ...
  form?.addEventListener('submit', function(e){
    ...
    if (!u) {
      if (!confirm('Bạn sắp gửi cho tất cả đối tác. Xác nhận gửi?')) { e.preventDefault(); return; }
    } else {
      if (!confirm('Gửi thông báo cho: ' + u + ' (khớp theo username hoặc company_name). Xác nhận gửi?')) { e.preventDefault(); return; }
    }
  });
})();
</script>
