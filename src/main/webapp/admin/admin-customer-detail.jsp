...
<head>
    ...
    <style>
        ...
        .customer-detail-container{max-width:1200px;margin:0 auto;padding:0 1rem;}
        ...
        .customer-header:before{content:none;} /* bỏ hiệu ứng tròn */
        .header-content{display:flex;justify-content:space-between;gap:1rem;align-items:flex-start;flex-wrap:wrap}

        .customer-identity{display:flex;align-items:center;gap:1rem}
        .avatar-xl{
        ...
        .customer-titles h1{margin:0 0 .25rem 0;font-size: clamp(20px,2.1vw,26px);font-weight:800;color:var(--text)}
        .customer-subtitle{display:flex;align-items:center;gap:.5rem;flex-wrap:wrap;opacity:.95}
        .customer-id{
            background: #eef2ff;
            border: 1px solid #e0e7ff;
            color:#334155;
            padding:.28rem .65rem;border-radius:999px;font-size:.82rem;font-weight:600
        }
        .header-actions{display:flex;gap:.6rem;flex-wrap:wrap}

        /* Grid */
        .customer-grid{display:grid; gap:1.25rem; grid-template-columns:minmax(0,1fr) 340px;}
        ...
        .info-row:hover{ background: #f8fafc; border-color: #e2e8f0; }
        .info-icon{width:18px; color: var(--primary); margin-top:.1rem}
        .info-content{flex:1; display:flex; justify-content:space-between; align-items:flex-start; gap:1rem}
        .info-label{min-width:140px; font-weight:700; color:#111827}
        ...
        .status-badge{
            ...
            letter-spacing:.3px;text-transform:uppercase;white-space:nowrap;border:1px solid transparent;
        }
        .status-badge.success{ background:#e9fbeF; border-color:#b7f0c7; color:#166534;}
        .status-badge.danger{  background:#fee2e2; border-color:#fecaca; color:#991b1b;}
        .status-badge.warning{ background:#fef3c7; border-color:#fde68a; color:#92400e;}
        .status-badge.info{    background:#e0f2fe; border-color:#bae6fd; color:#0c4a6e;}

        /* Buttons nhất quán theo 1 accent */
        .btn{
            appearance:none; border:none; cursor:pointer; text-decoration:none;
            display:inline-flex;align-items:center;gap:.5rem;
            padding:.7rem .95rem;border-radius:10px;font-weight:700;font-size:.9rem;
        ...
        .btn-warning:hover{ box-shadow: 0 8px 18px rgba(245,158,11,.25); }

        .btn-success{ color:#fff; background: #16a34a; }
        .btn-success:hover{ box-shadow: 0 8px 18px rgba(22,163,74,.25); }

        .btn-secondary{ color:#0f172a; background: #e5e7eb; }
        .btn-secondary:hover{ background:#dfe3e8; box-shadow: 0 6px 14px rgba(2,6,23,.08); }

        /* Wallet card: sắc nét, không gradient đậm */
        .wallet-card{
            position:relative; border-radius:12px; padding:14px 12px;
            background: #0f172a; color:#f8fafc;
            border:1px solid #0f172a;
            box-shadow: var(--shadow-1);
        }
        .wallet-card:after{content:none;}
        .wallet-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:.4rem}
        .wallet-title{font-weight:700}
        .wallet-balance{font-size: clamp(26px, 3vw, 36px); font-weight:900; margin:.2rem 0 .8rem}
        .wallet-details{display:grid; gap:.45rem}
        .wallet-item{display:flex; justify-content:space-between; padding:.55rem 0; border-bottom:1px dashed rgba(255,255,255,.15)}
        .wallet-item:last-child{border-bottom:none}
        .wallet-amount{font-weight:800}
        .wallet-amount.positive{color:#86efac}
        .wallet-amount.negative{color:#fecaca}
        .wallet-amount.zero{color:#e5e7eb}

        /* Stats */
        .stats-grid{display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:.9rem; margin-top:.2rem}
        .stat-card{
            text-align:center; border-radius:12px; padding:1rem;
            background: #ffffff;
            border:1px solid var(--line);
            transition: transform .1s ease, border-color .1s ease;
        }
        .stat-card:hover{ transform: translateY(-1px); border-color:#dbe1e8; }
        .stat-icon{width:44px;height:44px;border-radius:10px;display:grid;place-items:center;margin:0 auto .6rem;font-size:1.05rem}
        .stat-icon.wallet   {background: #e0f2fe; color:#0369a1}
        .stat-icon.orders   {background: #eafbea; color:#166534}
        .stat-icon.spent    {background: #fff7ed; color:#9a3412}
        .stat-icon.last-order{background:#eef2ff; color:#3730a3}
        .stat-value{font-weight:900; font-size:1.15rem;color:#111827}
        ...
        .data-table thead th{
            background:#f8fafc; color:#111827; text-align:left; font-weight:800; font-size:.86rem;
            padding: .85rem 1rem; border-bottom:1px solid var(--line); letter-spacing:.2px;
        }
        .data-table td{
            padding: .8rem 1rem; border-bottom:1px solid #f1f5f9; color:#111827;
        }
        ...
        .empty-state i{font-size:2.4rem; margin-bottom:.5rem; color:#cbd5e1}
        .empty-state h3{margin-bottom:.3rem;color:#111827}

        /* Flash */
        .flash-message{
            display:flex;align-items:center;gap:.55rem; padding:.85rem 1rem; border-radius:10px;
            border:1px solid var(--line); background:#ffffff; font-weight:700; color:#111827;
        }
        .flash-message.success{border-color:#bbf7d0; background:#ecfdf5; color:#065f46}
        .flash-message.error{  border-color:#fecaca; background:#fef2f2; color:#7f1d1d}

        /* Utilities */
        .panel{margin-bottom:1.25rem}
        .panel + .panel{margin-top:1.25rem}
        .table-container{margin-top:.8rem}
        ...
        @media (max-width: 820px){
            .info-content{flex-direction:column; gap:.4rem}
...
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="${pageContext.request.contextPath}/admin/customers" class="nav-item active"><i class="fas fa-users"></i><span>Customers</span></a>
...
<main class="content">
    ...
    <div class="customer-detail-container">
        ...
        <c:if test="${empty detail}">
            <div class="flash-message error"><i class="fas fa-exclamation-triangle"></i>Không tìm thấy thông tin khách hàng hoặc có lỗi xảy ra.</div>
            <div class="text-center" style="padding: 2rem;">
                <a href="${pageContext.request.contextPath}/admin/customers" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> Quay lại danh sách khách hàng</a>
            </div>
        </c:if>
                      ...
                      <c:if test="${empty detail.dob}">N/A</c:if>
                                ...
                                <div class="info-row">
                                    ...
                                    <div class="info-content">
                                        ...
                                        <span class="info-value"><fmt:formatDate value="${detail.createdAt}" pattern="dd/MM/yyyy HH:mm" /></span>
                                    </div>
                                </div>
                        ...
                        <div class="card-body">
                            <div class="stats-grid">
                                <div class="stat-card">
                                    ...
                                    <div class="stat-value"><fmt:formatNumber value="${detail.wallet}" type="currency" currencyCode="VND" /></div>
                                ...
                                <div class="stat-card">
                                    ...
                                    <div class="stat-value"><fmt:formatNumber value="${detail.totalSpent}" type="currency" currencyCode="VND" /></div>
