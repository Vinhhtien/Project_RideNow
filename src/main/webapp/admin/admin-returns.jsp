...
<head>
  ...
  <style>
    .badge { padding: 0.375rem 0.75rem; border-radius: 20px; font-size: 0.75rem; font-weight: 600; display: inline-flex; align-items: center; gap: 0.25rem; }
    ...
    .btn-sm { padding: 0.5rem 1rem; font-size: 0.875rem; display: inline-flex; align-items: center; gap: 0.375rem; }
...
<main class="content">
  ...
  <!-- Danh sách đơn chờ kiểm tra -->
  <section class="panel">
    <div class="panel-header">
      <h2><i class="fas fa-clock"></i> Đơn Hàng Chờ Kiểm Tra</h2>
      <div class="panel-stats"><span class="stat-badge"><i class="fas fa-list"></i> Tổng số: ${empty refundOrders ? 0 : refundOrders.size()}</span></div>
    </div>
            ...
            <table class="data-table">
              ...
              <c:forEach var="order" items="${refundOrders}">
                <tr>
                  ...
                  <td><strong style="color: #059669;"><fmt:formatNumber value="${order.depositAmount}" type="currency"/></strong></td>
                  <td><span class="badge returned"><i class="fas fa-check-circle"></i> Đã trả xe</span></td>
                  <td>
                    <a class="btn btn-primary btn-sm" href="${pageContext.request.contextPath}/adminreturninspect?orderId=${order.orderId}">
  ...
  <section class="panel">
    <div class="panel-header">
      ...
      <div class="panel-stats"><span class="stat-badge"><i class="fas fa-list"></i> Tổng số: ${empty refundRequests ? 0 : refundRequests.size()}</span></div>
    </div>
    <div class="panel-body">
      <c:choose>
            ...
            <table class="data-table">
              ...
              <c:forEach var="req" items="${refundRequests}">
                <tr>
                  ...
                  <td><strong style="color: #3b82f6;"><fmt:formatNumber value="${req.depositAmount}" type="currency"/></strong></td>
                  <td><strong style="color: #059669;"><fmt:formatNumber value="${req.refundAmount}" type="currency"/></strong></td>
                  <td>
                    <c:choose>
                  ...
                  <td>
                    <c:choose>
                      <c:when test="${req.bikeCondition eq 'excellent'}"><span class="badge excellent"><i class="fas fa-star"></i>Tốt</span></c:when>
                      <c:when test="${req.bikeCondition eq 'good'}"><span class="badge good"><i class="fas fa-check"></i>Bình thường</span></c:when>
                      <c:when test="${req.bikeCondition eq 'damaged'}"><span class="badge damaged"><i class="fas fa-tools"></i>Hư hỏng</span></c:when>
                  ...
                  <td>
                    <c:choose>
                      <c:when test="${req.status eq 'pending'}"><span class="badge pending"><i class="fas fa-clock"></i> Đang chờ</span></c:when>
                      <c:when test="${req.status eq 'processing'}"><span class="badge processing"><i class="fas fa-sync-alt"></i> Đang xử lý</span></c:when>
                    ...
                    <div class="action-buttons">
                      ...
                      <c:if test="${req.status eq 'pending'}">
                        <form method="post" action="${pageContext.request.contextPath}/adminreturns" style="display:inline;">
                          ...
                          <button type="submit" class="btn btn-success btn-sm" title="Duyệt yêu cầu">
                      ...
                      <!-- Hoàn tất: Về ví -->
                      <c:if test="${req.status eq 'pending' or req.status eq 'processing'}">
                        <form method="post" action="${pageContext.request.contextPath}/adminreturns" style="display:inline;">
