  ...
  <main class="content">
    ...
    <c:if test="${pendingCount == 0 && confirmedCount == 0 && completedCount == 0 && cancelledCount == 0}">
      ...
      <c:forEach var="o" items="${orders}">
        ...
        <c:if test="${o.orderStatus == 'confirmed'}"><c:set var="confirmedCount" value="${confirmedCount + 1}"/></c:if>
        <c:if test="${o.orderStatus == 'completed'}"><c:set var="completedCount" value="${completedCount + 1}"/></c:if>
        <c:if test="${o.orderStatus == 'cancelled'}"><c:set var="cancelledCount" value="${cancelledCount + 1}"/></c:if>
      </c:forEach>
    </c:if>
    ...
    <section class="order-toolbar">
      ...
      <div class="filter-tabs">
        ...
        <a href="${ctx}/admin/orders?status=pending&q=${fn:escapeXml(q)}&from=${from}&to=${to}" class="filter-tab ${status == 'pending' ? 'active' : ''}">
          <i class="fa-solid fa-clock"></i>
          Đang chờ
          <span class="count">${pendingCount}</span>
        </a>
        <a href="${ctx}/admin/orders?status=confirmed&q=${fn:escapeXml(q)}&from=${from}&to=${to}" class="filter-tab ${status == 'confirmed' ? 'active' : ''}">
          <i class="fa-solid fa-check-circle"></i>
          Đã xác nhận
          <span class="count">${confirmedCount}</span>
        </a>
        <a href="${ctx}/admin/orders?status=completed&q=${fn:escapeXml(q)}&from=${from}&to=${to}" class="filter-tab ${status == 'completed' ? 'active' : ''}">
          <i class="fa-solid fa-flag-checkered"></i>
          Hoàn thành
          <span class="count">${completedCount}</span>
        </a>
        <a href="${ctx}/admin/orders?status=cancelled&q=${fn:escapeXml(q)}&from=${from}&to=${to}" class="filter-tab ${status == 'cancelled' ? 'active' : ''}">
      ...
      <form class="filter-form" method="get" action="${ctx}/admin/orders">
        ...
        <div>
          ...
          <input type="text" id="customer-search" name="q" value="${fn:escapeXml(q)}" placeholder="Nhập tên khách hàng..."/>
        </div>
              ...
              <td>
                <c:choose>
                  <c:when test="${o.amountDue > 0}">
                    <span class="amount-negative"><fmt:formatNumber value="${o.amountDue}" type="currency"/></span>
                  </c:when>
                  <c:when test="${o.amountDue < 0}">
                    <span class="amount-positive"><fmt:formatNumber value="${-o.amountDue}" type="currency"/></span>
                  </c:when>
                  <c:otherwise>
                    <span class="amount-zero"><fmt:formatNumber value="${o.amountDue}" type="currency"/></span>
                  </c:otherwise>
                </c:choose>
              </td>
