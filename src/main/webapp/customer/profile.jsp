...
<head>
  ...
  <style>
    ...
    body { background:#0b132b; color:var(--text); font-family:Inter,system-ui; }

    .profile-wrap { max-width: 1100px; margin: 120px auto 40px; padding: 0 16px; }
    .profile-header { display:flex; align-items:center; gap:14px; margin-bottom:18px; }
    .profile-header i { color:var(--primary); }
    ...
    .card p.helper { color:var(--text-dim); margin-top:-6px; margin-bottom:14px; font-size:14px; }

    label { display:block; margin:10px 0 6px; color:#cbd5e1; font-weight:500; }
    input, textarea {
      width:100%; padding:12px 14px; border-radius:12px;
      border:1px solid #334155; background:#0f172a; color:#fff;
      outline:none;
    }
    input:focus, textarea:focus { border-color:var(--primary); box-shadow:0 0 0 3px rgba(91,192,190,.18); }
    ...
    .btn.ghost { background:transparent; border:1px solid #334155; color:#e2e8f0; font-weight:600; }

    .note { padding:12px 14px; border-radius:10px; margin:10px 0 14px; border-left:4px solid var(--ok); background:rgba(16,185,129,0.1); color:var(--ok); }
    .note.err { border-left-color:var(--err); background:rgba(239,68,68,.12); color:#fecaca; }
    .row-two { display:grid; grid-template-columns:1fr; gap:12px; }
    @media (min-width:680px){ .row-two{ grid-template-columns:1fr 1fr; } }
    .divider { height:1px; background: #1f2937; margin: 12px 0; }

    #header { position:fixed; top:0; left:0; right:0; z-index:100; }
    .profile-actions { display:flex; gap:10px; align-items:center; flex-wrap:wrap; margin-top:10px; }
  </style>
</head>
<body>
            ...
            <c:choose>
              <c:when test="${not empty sessionScope.account}">
                <a href="${ctx}/customer/profile" class="btn btn--ghost">
                  ...
                  Xin chào, <strong>${sessionScope.customerName != null ? sessionScope.customerName : sessionScope.account.username}</strong>
                </a>
        ...
        <form method="post" action="${ctx}/customer/profile" autocomplete="on">
          <div class="row-two">
            <div>
              ...
              <input id="full_name" name="full_name" type="text" value="${profile != null ? profile.fullName : ''}" required>
            </div>
            <div>
              <label for="email">Email</label>
              <input id="email" name="email" type="email" value="${profile != null ? profile.email : ''}" required>
            </div>
          </div>
          ...
          <div class="row-two">
            ...
            <div>
              ...
              <input id="address" name="address" type="text" value="${profile != null ? profile.address : ''}">
            </div>
          </div>
  ...
  <script>
    ...
    window.addEventListener('scroll',()=>{const h=$('#header'); if(window.scrollY>50)h.classList.add('scrolled'); else h.classList.remove('scrolled');});
  </script>
</body>
</html>
