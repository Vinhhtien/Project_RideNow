...
<head>
  ...
  <style>
    :root{--bg:#0f172a;--card:#1e293b;--text:#f1f5f9;--muted:#94a3b8;--primary:#3b82f6;--primary-2:#2563eb;--shadow:0 12px 36px rgba(2,6,23,.45);--radius:14px}
    *{box-sizing:border-box}
    ...
    .topbar{display:flex;justify-content:center;margin:16px 0}
    .btn{display:inline-flex;gap:8px;text-decoration:none;padding:10px 14px;border-radius:10px;font-weight:700}
    .btn-secondary{background:rgba(255,255,255,.06);color:#e2e8f0;border:1px solid rgba(255,255,255,.1)}
    .page-title{
      display:flex;justify-content:center;gap:8px;margin:24px 0 10px;font-weight:900;font-size:clamp(18px,2.2vw,24px);
      background:linear-gradient(90deg,#eaf2ff,#b7ceff,#7aa7ff);-webkit-background-clip:text;background-clip:text;color:transparent
    }
    .container{width:min(1200px,92%);margin:20px auto 40px;display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:22px}
    .card{
      background:linear-gradient(180deg,rgba(255,255,255,.05),rgba(255,255,255,.02)),var(--card);
      border:1px solid rgba(255,255,255,.07);border-radius:var(--radius);overflow:hidden;box-shadow:var(--shadow)
    }
    .thumb-wrap{width:100%;height:180px;overflow:hidden;border-bottom:1px solid rgba(255,255,255,.08);background:rgba(255,255,255,.03)}
    .card img{width:100%;height:180px;object-fit:cover;display:block}
    .card-body{text-align:center;padding:14px}
    .card-body h3{margin:8px 0 12px;font-size:16px;font-weight:800;color:#cfe0ff}
    .btn-detail{
      display:inline-flex;align-items:center;gap:6px;background:linear-gradient(135deg,var(--primary),var(--primary-2));
      color:#fff;padding:10px 16px;border-radius:10px;text-decoration:none;font-weight:700
    }
    .empty-message{text-align:center;margin:40px 0;color:var(--muted)}
  </style>
</head>
<body>
  <div class="topbar">
      ...
      <div class="container">
        <c:forEach var="bike" items="${motorbikes}">
          ...
          <c:if test="${imgFolder == 'khac' and not empty bike.typeName}">
            ...
            <c:if test="${fn:contains(typeLower,'số') or fn:contains(typeLower,'so')}"><c:set var="imgFolder" value="xe-so"/></c:if>
            <c:if test="${fn:contains(typeLower,'ga')}"><c:set var="imgFolder" value="xe-ga"/></c:if>
            <c:if test="${fn:contains(typeLower,'pkl') or fn:contains(typeLower,'phân khối') or fn:contains(typeLower,'phan khoi')}"><c:set var="imgFolder" value="xe-pkl"/></c:if>
          </c:if>
