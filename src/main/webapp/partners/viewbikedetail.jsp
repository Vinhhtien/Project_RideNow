...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .kv b{flex:0 0 clamp(120px, 22%, 180px); color:#d8e1ff;font-weight:800;letter-spacing:.2px;opacity:.95}
    ...
    .btn-primary{background:linear-gradient(180deg,#9fb7ff,#6c8cff); color:#081028;border-color:#90adff;text-shadow:0 1px 0 rgba(255,255,255,.4)}
    .btn-ghost{border:1px dashed #3a4477}

    .rv-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:14px}
    .rv{display:grid;grid-template-columns:88px 1fr;gap:12px;border:1px solid rgba(255,255,255,.08);border-radius:12px;padding:12px;background:rgba(255,255,255,.03)}
    ...
    .empty{padding:16px;border:1px dashed rgba(255,255,255,.12); border-radius:12px;color:var(--muted);text-align:center;background:rgba(255,255,255,.02)}
    @media (prefers-reduced-motion:reduce){.card,.btn,.hero img,.rv{transition:none}}
  </style>
</head>
<body>
  <div class="wrap">
    ...
    <c:choose>
      ...
      <c:when test="${fn:contains(statusLower,'rent') or fn:contains(statusLower,'hold')}"><c:set var="statusCls" value="warn"/></c:when>
      <c:when test="${fn:contains(statusLower,'maintenance') or fn:contains(statusLower,'maintain')}"><c:set var="statusCls" value="bad"/></c:when>
                  ...
                  <div class="meta">
                    <c:choose>
                      <c:when test="${not empty rv.createdAt}">
                        <c:catch var="__fmtErr"><fmt:formatDate value="${rv.createdAt}" pattern="HH:mm dd/MM/yyyy" var="__fmtOut"/></c:catch>
                        <c:choose>
                          <c:when test="${empty __fmtErr and not empty __fmtOut}"><c:out value="${__fmtOut}"/></c:when>
                          <c:otherwise>
                            <c:set var="__printed" value="false"/>
                            <c:catch var="__p1err"><fmt:parseDate value="${rv.createdAt}" pattern="yyyy-MM-dd'T'HH:mm:ssX" var="__p1"/></c:catch>
                            <c:if test="${empty __p1err and not empty __p1}"><fmt:formatDate value="${__p1}" pattern="HH:mm dd/MM/yyyy"/><c:set var="__printed" value="true"/></c:if>
                            <c:if test="${not __printed}">
                              <c:catch var="__p2err"><fmt:parseDate value="${rv.createdAt}" pattern="yyyy-MM-dd'T'HH:mm:ss" var="__p2"/></c:catch>
                              <c:if test="${empty __p2err and not empty __p2}"><fmt:formatDate value="${__p2}" pattern="HH:mm dd/MM/yyyy"/><c:set var="__printed" value="true"/></c:if>
                            </c:if>
                            <c:if test="${not __printed}">
                              <c:catch var="__p3err"><fmt:parseDate value="${rv.createdAt}" pattern="yyyy-MM-dd HH:mm:ss" var="__p3"/></c:catch>
                              <c:if test="${empty __p3err and not empty __p3}"><fmt:formatDate value="${__p3}" pattern="HH:mm dd/MM/yyyy"/><c:set var="__printed" value="true"/></c:if>
                            </c:if>
                            <c:if test="${not __printed}"><c:out value="${rv.createdAt}"/></c:if>
                          </c:otherwise>
                        </c:choose>
                      </c:when>
