...
<head>
  ...
  <style>
    :root{
      --bg:#0b0f1f; --bg2:#0a0d1c; --card:#121735; --card2:#10152e;
      --text:#e8eaf6; --muted:#a7b0d6; --border:#1c244c; --shadow:0 10px 30px rgba(0,0,0,.35);
      --primary:#6c8cff; --accent:#22d3ee; --danger:#ff6b6b; --success:#34d399; --warn:#f59e0b;
      --radius:16px;
    }
    *{box-sizing:border-box} html,body{margin:0}
    ...
    .btn.primary{background:var(--accent);color:#0f172a;border-color:#24cde0}
    .card{
    ...
    /* Table */
    table{width:100%;border-collapse:collapse;background:var(--card2);border:1px solid var(--border);border-radius:12px;overflow:hidden}
    thead th{background:#0f142e;color:#b9c7ff;font-weight:800;text-align:left;padding:12px;border-bottom:1px solid var(--border);font-size:13px;letter-spacing:.3px}
    tbody td{padding:12px;border-bottom:1px solid #222b57;vertical-align:top}
    tbody tr:hover{background:#101735}
    .right{text-align:right}
    .muted{color:var(--muted)}
    .badge{
      display:inline-block;border-radius:999px;padding:4px 10px;font-size:11px;font-weight:800;letter-spacing:.3px;
      border:1px solid #3a4477;
    }
    .ok{background:rgba(52,211,153,.15);color:#b9f9de;border-color:rgba(52,211,153,.35)}
    .bad{background:rgba(255,107,107,.15);color:#ffd0d0;border-color:rgba(255,107,107,.35)}
    .warn{background:rgba(245,158,11,.18);color:#ffe4b5;border-color:rgba(245,158,11,.35)}

    /* Highlight when coming from ?orderId= */
    tr.hl{outline:2px solid var(--accent)}
    ...
    .modal .dialog{
      ...
      width:min(680px, 94%);background:var(--card);border:1px solid var(--border);
      border-radius:16px;box-shadow:var(--shadow);padding:16px 16px 12px;
    }
    .dialog h3{margin:2px 0 2px}
    .dialog .meta{color:var(--muted);font-size:13px;margin-bottom:8px}
    .dialog .table{border:1px solid var(--border);border-radius:12px;overflow:hidden}
    .dialog thead th{background:#0f142e}
    .dialog .foot{display:flex;gap:10px;justify-content:flex-end;margin-top:12px}
  </style>
</head>
<body>
  <c:set var="orders" value="${not empty displayOrders ? displayOrders : (not empty rentalList ? rentalList : histories)}" />
  <div class="wrap">
    <div class="top">
            ...
            <tbody>
              <c:forEach var="o" items="${orders}">
                ...
                <tr class="${rowClass}"
                  ...
                  <td>
                    <div><c:out value="${o.customerName != null ? o.customerName : 'Khách #'+o.customerId}"/></div>
                    <div class="muted">Xe:
                  ...
                  <td>
                    <button class="btn primary" onclick="showDetail(<c:out value='${o.orderId}'/>)">Xem</button>
                  </td>
                </tr>
    ...
    <div class="dialog" role="dialog" aria-label="Chi tiết đơn hàng">
      ...
      <div class="table">
        <table>
          ...
          <tbody id="popupBody">
            <tr><td colspan="4" class="muted" style="padding:12px">Không có chi tiết.</td></tr>
          </tbody>
        </table>
      </div>
  ...
  <script>
    ...
    (function(){ var q = document.getElementById('q'); if(q){ q.addEventListener('input', filterRows); } })();
    ...
    function showDetail(orderId){
      ...
      var items = [];
      if(sc){
        try { items = JSON.parse(sc.textContent); } catch(e){ items = []; }
      }
    ...
    function openPopup(){ var m = document.getElementById('popup'); m.classList.add('open'); m.setAttribute('aria-hidden','false'); }
    function closePopup(){ var m = document.getElementById('popup'); m.classList.remove('open'); m.setAttribute('aria-hidden','true'); }
