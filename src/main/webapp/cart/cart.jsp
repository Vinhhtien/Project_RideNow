...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    body{font-family:'Inter','Segoe UI',Tahoma,Geneva,Verdana,sans-serif;background:linear-gradient(135deg,#0a0b0d 0%,#121318 100%);color:var(--light);line-height:1.6;margin:0;min-height:100vh}
    .container{max-width:1200px;margin:0 auto;padding:40px 20px}
    header{background:rgba(11,11,13,.94);backdrop-filter:blur(10px);box-shadow:var(--shadow-md);position:sticky;top:0;z-index:100;border-bottom:1px solid var(--primary-light)}
    .header-content{display:flex;justify-content:space-between;align-items:center;padding:16px 20px;max-width:1200px;margin:0 auto}
    .brand{display:flex;align-items:center;gap:12px;text-decoration:none;color:var(--accent);font-weight:800;font-size:26px;letter-spacing:-.5px;text-shadow:0 0 10px rgba(59,130,246,.25)}
    .brand img{height:40px;border-radius:4px}
    .auth{display:flex;gap:12px}
    .btn{display:inline-flex;align-items:center;gap:8px;padding:8px 16px;border-radius:var(--radius);text-decoration:none;font-weight:600;transition:var(--transition);border:1px solid var(--primary-light);background:transparent;color:var(--light)}
    .btn--ghost:hover{background:var(--primary-light);color:var(--accent);border-color:var(--accent)}
    .btn-gray{background:var(--secondary);border:1px solid var(--primary-light);border-radius:8px;padding:8px 12px;color:#e5e7eb;cursor:pointer;transition:var(--transition)}
    .btn-gray:hover{background:rgba(59,130,246,.15);color:#93c5fd;border-color:rgba(59,130,246,.3)}
    .btn-danger{background:transparent;border:1px solid #ef4444;color:#fca5a5;border-radius:8px;padding:8px 12px;cursor:pointer}
    .btn-danger:hover{background:rgba(239,68,68,.15)}
    .page-title{font-size:2rem;font-weight:800;margin:0 0 24px;color:var(--accent);text-shadow:0 0 10px rgba(59,130,246,.25);display:flex;align-items:center;gap:12px}
    .card{background:var(--dark-light);border-radius:12px;box-shadow:var(--shadow-md);padding:24px;margin-bottom:20px;border:1px solid var(--primary-light)}
    .table-container{overflow-x:auto;border-radius:8px}
    .table{width:100%;border-collapse:collapse;background:transparent}
    .table th,.table td{padding:16px;text-align:left;border-bottom:1px solid var(--primary-light);vertical-align:top}
    .table th{background:var(--secondary);font-weight:600;color:var(--accent);font-size:.9rem;text-transform:uppercase;letter-spacing:.5px}
    .table tr:last-child td{border-bottom:none}
    .table tr:hover{background:rgba(59,130,246,.05)}
    .bike-name{font-weight:600;color:#fff}
    .bike-type{color:#cbd5e1;font-size:.9rem}
    .date-row{display:flex;gap:8px;align-items:center;flex-wrap:wrap}
    .date-row input[type="date"]{background:#6f6f6f;color:#fff;border:1px solid var(--primary-light);border-radius:8px;padding:8px 10px;min-width:150px}
    .field-hint{font-size:.85rem;color:#9aa2b2}
    .actions{display:flex;justify-content:flex-end;margin-top:20px}
    .summary-card{width:100%;max-width:420px;background:var(--dark-light);border-radius:12px;padding:20px;box-shadow:var(--shadow-md);border:1px solid var(--primary-light)}
    .summary-row{display:flex;justify-content:space-between;margin-bottom:12px;padding-bottom:12px;border-bottom:1px solid var(--primary-light)}
          ...
          <table class="table" id="cartTable">
            ...
            <c:forEach var="it" items="${cartItems}" varStatus="st">
              ...
              <tr data-row="${st.index}">
                ...
                <td><span class="js-price" data-price="${it.pricePerDay}"><fmt:formatNumber value="${it.pricePerDay}" type="number"/> đ</span></td>
                <td><span class="js-days">${it.days}</span></td>
                <td><span class="js-subtotal" data-sub="${it.subtotal}"><fmt:formatNumber value="${it.subtotal}" type="number"/> đ</span></td>
                <td><span class="js-deposit" data-deposit="${it.deposit}"><fmt:formatNumber value="${it.deposit}" type="number"/> đ</span></td>
...
function recalcRow(tr){
  ...
  if (!start || !end) return;
  if (start < today){
    errEl.textContent = 'Ngày nhận không được ở quá khứ.'; errEl.style.display = 'block'; return;
  }
  if (end < start){
    errEl.textContent = 'Ngày trả phải sau hoặc bằng ngày nhận.'; errEl.style.display = 'block'; return;
  }
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  if (table) {
    table.addEventListener('click', (ev) => {
      ...
      if (!tr) { console.warn('Không tìm thấy <tr> chứa nút Save.'); return; }

      const startEl = tr.querySelector('.js-start');
      const endEl   = tr.querySelector('.js-end');
      const errEl   = tr.querySelector('.js-error');

      if (!startEl || !endEl) { console.warn('Không tìm thấy input date trong hàng.'); return; }
      ...
      if (!startVal || !endVal){ if (errEl){ errEl.textContent='Vui lòng chọn đủ ngày.'; errEl.style.display='block'; } return; }
      if (startVal < today){ if (errEl){ errEl.textContent='Ngày nhận không được ở quá khứ.'; errEl.style.display='block'; } return; }
      if (endVal < startVal){ if (errEl){ errEl.textContent='Ngày trả phải sau hoặc bằng ngày nhận.'; errEl.style.display='block'; } return; }
